...
control ingress(inout headers hdr, inout metadata meta, inout standard_metadata_t standard_metadata) {
    ...
    register<bit<32>>(20) current_seq_no;
    register<bit<32>>(20) decision_seq_no;

    // Decision table
    register<bit<32>>(20) decision_f1;       // Function value
    register<bit<32>>(20) decision_f2;       // Function value

    // Choices table
    register<bit<9>>(20) choices_nhop;      // Next hop port
    register<bit<32>>(20) choices_f1;        // Function value
    register<bit<32>>(20) choices_f2;        // Function value
    ...
    register<bit<48>>(4) link_time;              // time of last probe using the link

    // Metric length
    register<bit<32>>(3) local_length;     // Local length per port.

    // Metric util
    register<bit<32>>(3) local_util;     // Local util per port.
    register<bit<48>>(3) last_packet_time;
...
//        local_length.write((bit<32>) 0, 1);
//        local_length.write((bit<32>) 1, 1);
        ...
        if (hdr.hulapp_data.isValid()
            || (hdr.tcp.isValid() || hdr.udp.isValid())
                 && (standard_metadata.ingress_port == 1
                     || standard_metadata.ingress_port >= 4)          // TCP/UDP data packet from outside
            || hdr.arp_ipv4.isValid()
                 && (standard_metadata.ingress_port == 1
                     || standard_metadata.ingress_port >= 4)) {  // arp request/reply from outside
            ...
            if (hdr.ipv4.isValid()) {
                ...
                bool maybe_loop = (fcount + LOOP_THRESHOLD < hop_count);
            ...
            if (hdr.ipv4.protocol == HULAPP_BACKGROUND_PROTOCOL && standard_metadata.ingress_port == 1) {
                ...
                if (standard_metadata.ingress_port != 1) {
                    local_length.read(len, (bit<32>) standard_metadata.ingress_port - 2);
                }
                hdr.hulapp.length = hdr.hulapp.length + len;
                ...
                bit<32> tmp_length0 = hdr.hulapp.length;

                bit<32> probe_decision_f1 = tmp_length0;
                bit<32> probe_decision_f2 = tmp_util0;

                // Compute the probe-local, choices evaluation function
                bit<32> probe_f1 = 9999999;
                bit<32> probe_f2 = 9999999;
                    probe_f1 = hdr.hulapp.length;
                    probe_f2 = hdr.hulapp.util;
                ...
                bool better_f = (probe_f1 < x1 || (probe_f1 == x1 && (probe_f2 < x2)));
                ...
                better_f = (probe_decision_f1 < x1 || (probe_decision_f1 == x1 && (probe_decision_f2 < x2)));
                if ((eq_seq && better_f) || gt_seq) {
                    decision_f1.write(dst, probe_decision_f1);
                    decision_f2.write(dst, probe_decision_f2);
