...
control ingress(inout headers hdr, inout metadata meta, inout standard_metadata_t standard_metadata) {
    ...
    // Metric length
    register<bit<32>>(7) local_length;     // Local length per port.
...
//        local_length.write((bit<32>) 4, 1);
//        local_length.write((bit<32>) 5, 1);
        ...
        if (hdr.hulapp_data.isValid()
            || (hdr.tcp.isValid() || hdr.udp.isValid())
                 && (standard_metadata.ingress_port == 1
                     || standard_metadata.ingress_port >= 8)          // TCP/UDP data packet from outside
            || hdr.arp_ipv4.isValid()
                 && (standard_metadata.ingress_port == 1
                     || standard_metadata.ingress_port >= 8)) {  // arp request/reply from outside
