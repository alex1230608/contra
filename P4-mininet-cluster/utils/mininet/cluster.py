...
class RemoteP4Host( RemoteMixin, P4Host ):
    "Remote instance of P4Host"
    pass

class RemoteP4Switch( RemoteMixin, P4Switch ):
    "Remote instance of P4Switch"
    ...
    def start(self, controllers):
        ...
        with tempfile.NamedTemporaryFile() as f:
            #self.rcmd(' '.join(args) + ' > /dev/null 2>&1 &')
#            info(' '.join(args) + '\n')
            p = self.rpopen((' '.join(args)).split(), stdout=self.output)
            #pid = int(f.read())
        # debug("P4 switch {} PID is {}.\n".format(self.name, pid))
        sleep(1)
        if not self.check_switch_started(p.pid):
    ...
    def check_switch_started(self, pid):
        ...
        while True:
            ...
            sock.settimeout(0.5)
            result = sock.connect_ex((self.server, self.thrift_port))
            if result == 0:
                return  True
    ...
    def stop(self):
        "Terminate P4 switch."
        self.output.flush()
        self.rpopen(('pkill ' + self.sw_path).split(), sudo = True)
        # self.rpopen('wait')
        self.deleteIntfs()
...
class UserdefinedPlacer( Placer ):
    """User-defined placement
       Take user's input for each host and switch to place them
       in the specified servers"""

    def __init__( self, *args, **kwargs ):
        self.hostsConfig = kwargs.pop( 'hostsConfig', None )
        self.switchesConfig = kwargs.pop( 'switchesConfig', None )
    ...
    def place( self, nodename ):
        if nodename in self.hset:
            x = int(self.hostsConfig[nodename]['server']) if 'server' in self.hostsConfig[nodename] else randrange( 0, len( self.servers )-1 ) 
        elif nodename in self.sset:
            x =  int(self.switchesConfig[nodename]['server']) if 'server' in self.switchesConfig[nodename] else randrange( 0, len( self.servers )-1 ) 
        else:
            x = randrange( 0, len (self.servers)-1 )
        return self.servers[ x ]
...
class MininetCluster( Mininet ):
    ...
    def __init__( self, *args, **kwargs ):
        ...
        self.hostsConfig = params.pop( 'hostsConfig', None )
        self.switchesConfig = params.pop( 'switchesConfig', None )
    ...
    def placeNodes( self ):
        ...
        if self.placement == UserdefinedPlacer:
            placer = self.placement( servers=self.servers,
                                     nodes=self.topo.nodes(),
                                     hosts=self.topo.hosts(),
                                     switches=self.topo.switches(),
                                     links=self.topo.links(),
                                     hostsConfig=self.hostsConfig,
                                     switchesConfig=self.switchesConfig )
        else:
            placer = self.placement( servers=self.servers,
                                 nodes=self.topo.nodes(),
                                 hosts=self.topo.hosts(),
...
def testRemoteNet( remote='ubuntu2' ):
    ...
    net = Mininet( host=RemoteHost, switch=RemoteP4Switch,
                   link=RemoteLink )
...
def SwitchPlacer( name, *args, **params ):
    ...
    if name in remoteSwitches:
        return RemoteP4Switch( name, *args, server=remoteServer, **params )
    else:
        return RemoteP4Switch( name, *args, **params )
