...
module File = 
   ...
   let writeFile dir text = 
      System.IO.File.WriteAllText(dir, text)

   let appendFile dir text = 
      System.IO.File.AppendAllText(dir, text)
...
module String = 
   ...
   let join (ls : seq<string>) (sep : string) : string =
      match Seq.length ls with 
      | 0 -> ""
      | 1 -> Seq.head ls 
      | _ ->
         let str = Seq.fold (fun acc l -> acc + sep + l) (Seq.head ls) (Seq.skip 1 ls)
         str
   ...
   let private parseDecimal x = Decimal.Parse(x, Globalization.CultureInfo.InvariantCulture)
